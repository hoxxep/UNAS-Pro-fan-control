...
# Run ON the device, as root. Also removes the sensors.sh diagnostic that
...
# sensors.sh is only on the device because install.sh put it there, so it goes
# too -- but only once its header confirms it is ours, in case something else
# has since claimed the path.
